...
int Monopoly::GetPropToUpgrade(std::vector<Space *> &upgradeableSpaces) {
  int choice;
  if (upgradeableSpaces.empty()) {
    ...
    for (int i = 0; i < static_cast<int>(upgradeableSpaces.size()); i++) {
      std::cout << i << ". " << upgradeableSpaces[i]->GetName() << " [$" << dynamic_cast<Space::Property*>(upgradeableSpaces[i])->CalcUpgradeCost() << ']' << std::endl;
    }
...
int Monopoly::GetPropToDowngrade(std::vector<Space *> &downgradeableSpaces) {
  int choice;
  if (downgradeableSpaces.empty()) {
    ...
    for (int i = 0; i < static_cast<int>(downgradeableSpaces.size()); i++) {
      std::cout << i << ". " << downgradeableSpaces[i]->GetName() << " [$" << dynamic_cast<Space::Property*>(downgradeableSpaces[i])->CalcValDowngrade() << ']' << std::endl;
    }
    std::cout << "Your choice: ";
  }
  std::cin >> choice;
...
int Monopoly::EmergencyDowngrade(int owed, int current, std::vector<Space *> &downgradeableSpaces) {
  int choice;
  if (downgradeableSpaces.empty()) {
    return -1;
  } else {
