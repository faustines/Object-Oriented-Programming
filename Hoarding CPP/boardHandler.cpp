...
Monopoly::Board::Board(char *filename) {
  int numSpaces;
  file.open(filename);
  file.ignore(256, ',');
  file >> numSpaces;
...
std::vector<int> Monopoly::Board::GetUniqueIDs() {
  std::map<int, bool> checked;
  std::vector<int> ids;
  for (const std::shared_ptr<Space> &space : board) {
    std::shared_ptr<Space::Property> prop = std::dynamic_pointer_cast<Space::Property>(space);
    if (space->GetType() == SpaceType::Property
        && checked.count(prop->GetSetID()) == 0) {
